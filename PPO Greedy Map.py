...
class CIFAR10WindowEnv(gym.Env):
    def __init__(self, data, labels, patch_size=8, max_steps=15, log_file="agent_movement_log_2.log", epsilon=0.1):
        super().__init__()
        self.data = data
        self.labels = labels
        ...
        self.epsilon = epsilon  # Вероятность случайного действия (ε-greedy)
        
        # Массив для отслеживания исследованных координат
        self.visited_map = np.zeros((32, 32), dtype=np.int32)  # Изменили на np.int32 для подсчёта посещений

        if not os.path.exists(self.log_file):
            with open(self.log_file, 'w') as log:
                log.write("step, x, y, zoom, label, image_index, visited_map\n")

        self.action_space = spaces.MultiDiscrete([
            2, 2, 2, 2,  # движения (вверх, вниз, влево, вправо)
            2, 2,        # зум (вход, выход)
            10            # классификация
        ])
    ...
    def step(self, action):
        # С применением стратегии ε-greedy:
        if random.random() < self.epsilon:  # С вероятностью ε выбираем случайное действие
            action = self.action_space.sample()
        
        mv_up, mv_dn, mv_lf, mv_rt, zi, zo, cls = action  # извлекаем действия
        reward = 0.0
        done = False

        # Логика перемещения с диагональными движениями
        self.x = np.clip(self.x + mv_rt - mv_lf, 0, 32 - self.patch_size)
        self.y = np.clip(self.y + mv_dn - mv_up, 0, 32 - self.patch_size)

        # Обновление карты посещений
        end_x = min(self.x + self.patch_size, 32)
        end_y = min(self.y + self.patch_size, 32)
        self.visited_map[self.x:end_x, self.y:end_y] = True

        # Проверка на новую область
        new_visited = not self.visited_map[self.x:end_x, self.y:end_y].any()
        reward = 0.1 if new_visited else 0.0  # Положительная награда за исследование новой области

        # Система бонусов за продолжительное исследование
        #consecutive_steps = np.count_nonzero(self.visited_map == 1)
        #reward = 0.1 if consecutive_steps > 5 else 0.0 

        # Логика изменения зума
        if zi:  # Увеличение зума
            self.zoom = min(self.max_zoom, self.zoom * 2)
        if zo:  # Уменьшение зума
            self.zoom = max(self.min_zoom, self.zoom // 2)

        self.steps += 1

        # Награда за исследование и классификацию
    ...
    def _get_patch(self):
        p = self.patch_size * self.zoom  # Изменяем размер патча в зависимости от зума
...
log_file = "agent_movement_log_2.log"
train_env_fns = [
...
entropy_coeffs = np.array([0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.01])
